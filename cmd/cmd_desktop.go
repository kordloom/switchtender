...
// desktopCmd runs SwitchTender as a local desktop application: it serves on a loopback port, stores
...
func runDesktop(cmd *cobra.Command, _ []string) error {
	...
	if port, ok := savedDesktopPort(dir); ok && desktopAlive(port) {
		...
		fmt.Fprintln(os.Stderr, "SwitchTender is already running at "+url)
	...
	fmt.Fprintln(os.Stderr, "SwitchTender is starting at "+url)
...
func desktopDataDir() (string, error) {
	...
	dir := filepath.Join(base, "SwitchTender")
...
// browser. Setting SWITCHTENDER_DESKTOP_NO_BROWSER to any value skips the open, for a headless or
...
func openWhenReady(addr, url string) {
	if os.Getenv("SWITCHTENDER_DESKTOP_NO_BROWSER") != "" {
		return
	}
	for range 100 {
