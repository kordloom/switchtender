...
Yardmaster runs a complete control plane with no model anywhere near it. AI is an optional layer
...
| `anthropic` | Anthropic's API | `--ai-provider anthropic --ai-model claude-sonnet-5` plus `YARDMASTER_AI_KEY`. |
| `openai` | Any OpenAI-compatible endpoint | `--ai-provider openai --ai-model <model> --ai-url <server>` plus `YARDMASTER_AI_KEY`. Covers OpenAI itself, vLLM, LM Studio, and OpenRouter. |
...
When the Anthropic model is a Fable or Mythos model, Yardmaster opts into server-side fallbacks,
