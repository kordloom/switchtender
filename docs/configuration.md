...
Railwarden is one binary with subcommands. This page lists every command, flag, and environment
variable.
...
|----------|---------|---------|
| `RAILWARDEN_ENCRYPTION_KEY` | serve, worker | Passphrase that seals stored credentials with AES-256-GCM. Credentials are disabled when unset. |
| `RAILWARDEN_ENCRYPTION_SALT` | serve, worker | Per-deployment salt for argon2id key derivation. Must be set alongside the key and stay stable across restarts, or stored credentials cannot be decrypted. Credentials are disabled when unset. |
| `RAILWARDEN_AUDIT_KEY` | serve | Hex-encoded ed25519 seed that signs audit exports so the trail can be verified offline. Signing is off when unset. A malformed value stops startup. Generate one with `railwarden audit keygen`. |
| `RAILWARDEN_PASSWORD` | user new | Initial account password, read instead of prompting so it never lands on the command line. |
| `RAILWARDEN_SMTP_PASSWORD` | serve | Password for SMTP authentication when `--smtp-username` is set. |
| `RAILWARDEN_AI_KEY` | serve | API key for a cloud AI provider such as Anthropic or an OpenAI-compatible endpoint. A local Ollama needs none. |
| `RAILWARDEN_OIDC_CLIENT_SECRET` | serve | OpenID Connect client secret, paired with `--oidc-client-id`. Read from the environment so it stays off the command line. |
| `RAILWARDEN_LDAP_PASSWORD` | serve | Password for the `--ldap-bind-dn` service account. |
| `RAILWARDEN_ADMIN_PASSWORD` | init | Password for the first admin account. When unset, init generates one and prints it once. |
| `RAILWARDEN_DESKTOP_NO_BROWSER` | desktop | Set to any value to skip opening the browser, for a headless or remote run. |
...
|------|---------|---------|
| `--db` | `railwarden.db` | SQLite database path. |
| `--config` | `railwarden.env` | Environment file to write. |
...
The admin password comes from `RAILWARDEN_ADMIN_PASSWORD`, or is generated and printed once when
...
|------|---------|---------|
| `--addr` | `:8080` | Address the server listens on. |
| `--db` | `railwarden.db` | SQLite file path, or a `postgres://` DSN for the PostgreSQL backend. |
...
| `--plugins-dir` | none | Directory of extension plugin binaries loaded at startup. Also `RAILWARDEN_PLUGINS_DIR`. See [Extend in Go](sdk.md). |
...
| `--smtp-username` | none | SMTP username. The password comes from `RAILWARDEN_SMTP_PASSWORD`. |
...
model, or `anthropic` and `openai` for a cloud model with `RAILWARDEN_AI_KEY`. The provider is off
...
benign request as a false positive. When the Anthropic model is a Fable or Mythos model, Railwarden
...
Runs Railwarden as a local desktop application: it serves on a private loopback port, stores its
data in a per-user directory, and opens the web UI in the default browser. It takes no flags. Set
`RAILWARDEN_DESKTOP_NO_BROWSER` to skip opening a browser. See [Desktop](desktop.md) for packaging a
...
| Flag | Default | Purpose |
|------|---------|---------|
| `--db` | `railwarden.db` | SQLite file path, or a `postgres://` DSN. |
...
| `--plugins-dir` | none | Directory of extension plugin binaries loaded at startup. Also `RAILWARDEN_PLUGINS_DIR`. |
| `--container-memory` | `2g` | Memory cap for containerized runs, as docker `--memory`. Empty removes the cap. |
| `--container-cpus` | `2` | CPU cap for containerized runs, as docker `--cpus`. Empty removes the cap. |
| `--container-pids-limit` | `2048` | Process cap for containerized runs, as docker `--pids-limit`. Zero removes the cap. |
...
- `user new <username> --role <role>` creates an account. The password comes from
  `RAILWARDEN_PASSWORD` or a prompt, never an argument.
