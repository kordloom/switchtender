...
| Policy as code | Point `--policy-file` at a YAML file and it becomes the source of truth for policies. A rule can hold a run for approval or deny it outright (`effect: deny`), can scope itself to agents or people (`actor_kind`), to one named principal (`actor`), or to a risk floor (`min_risk`), so an agent's authorization boundary is a reviewed diff with a commit behind it. The API refuses policy writes with a 409 that says where to make the change, and a malformed file stops the server rather than silently gating nothing.|
...
| Saved workflows | A template can carry a pipeline graph instead of a single tool, so a multi-step workflow with parallel branches, per-step tools, and retries is one saved object that a launch, a schedule, or a webhook fires the same way, and its survey answers reach every step. *Next release.* |
