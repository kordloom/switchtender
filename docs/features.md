...
| Workers (Team) | Point `switchtender worker` at the same database and it competes for queued runs. Leases, heartbeats, and a janitor make dead workers safe.|
...
| Storage      | SQLite by default. The same flag takes a PostgreSQL DSN for multi-instance. Creating a new PostgreSQL schema needs Team; opening one that already exists is never gated.|
...
| Auth         | User accounts with admin, operator, and viewer roles enforced per route. Bearer tokens hashed at rest. A public bind with no tokens mints an initial admin token at startup and prints it once, so the API is never served open on a network interface; only loopback and read-only may run open, with a warning.|
| Approvals    | Mark a run to require sign-off, or require it automatically by policy on tool, command, target, assessed risk, or who is asking. Community holds one such policy, and matching on risk or on who is asking is part of the full engine, which is Team. A held run never executes until an admin approves or rejects it. The decision is a chain entry committing a digest of the exact spec released, the executor refuses a spec that changed after the decision, and the receipt proves the approved and executed change are the same.|
| Reversibility | Every run is graded on whether it can be taken back: reversible, costly, or irreversible. It is a different question from risk, and the two come apart constantly: restarting a fleet is high risk and undoes itself, while deleting a backup set reads quietly and is permanent. An Ansible run is graded from its playbook's own text, walking plays, tasks, and blocks, and a finished run is graded from what it actually did, so a run that changed nothing has nothing to undo. An approval policy can hold on the grade, which is the rule most people want and could not write: a change nobody can take back needs a second person to agree to it.|
| Estate history | Every fact gather is kept rather than overwritten, so the estate can be reconstructed as it stood on any date since the install. `GET /v1/estate?at=` answers what was running then; `GET /v1/estate/diff?from=&to=` answers what moved since, host by host and fact by fact, with the value at each end. Both report how far back the records reach, so an empty answer is never mistaken for an empty estate. Depth and spacing are configurable.|
| Changes | Label runs with a change and read the whole arc as one thing: `GET /v1/changes/{name}` returns every run that served one intent, its span, everyone who acted, and an outcome derived from the runs rather than declared. A rollback followed by a fix-forward reads as one mixed change, which is what really happened and what a single run can never express.|
| Policy as code (Team past one plain rule) | Point `--policy-file` at a YAML file and it becomes the source of truth for policies. A rule can hold a run for approval or deny it outright (`effect: deny`), can scope itself to agents or people (`actor_kind`), to one named principal (`actor`), or to a risk floor (`min_risk`), and can demand that the approver be someone other than the requester (`require_distinct_approver`), so an agent's authorization boundary is a reviewed diff with a commit behind it. The API refuses policy writes with a 409 that says where to make the change, and a malformed file stops the server rather than silently gating nothing.|
| Observability| A Prometheus metrics endpoint, webhook notifications when runs finish, and an audit trail of every mutation.|
| Tamper-evident audit | Every mutation is linked into a SHA-256 hash chain. `GET /v1/audit/verify` flags the first altered or deleted entry, and a signed LoomSeal bundle proves the whole chain offline to a third party running an open verifier this project does not control. A change that cannot be recorded is refused rather than performed silently.|
...
| Evidence report (Team) | `switchtender audit report` renders the period's changes into a self-contained HTML compliance report a vendor-security or SOC 2 reviewer reads with no tooling. The chain behind it is proved separately by verifying a signed bundle, so the narrative and the proof are independent artifacts.|
...
| Scheduled evidence packs (Team) | `--evidence-dir` with `--evidence-cadence` writes a change register covering each elapsed period, named for the period to the second, so an archive sorts chronologically and a missing period is a missing file. Progress lives in the archive rather than in memory, so a restart resumes where the last pack ended and a period that failed to write is covered by the next attempt instead of being skipped. The evidence a review samples from exists before it is asked for.|
| Change register (Team) | `switchtender audit report --from --to`, or the audit page's Evidence pack button, renders every change in the period with its actor, chain-recorded decision, risk grade, and outcome. That is the change-management sample a SOC 2 CC8.1 or ISO/IEC 27001 A.8.32 review asks for.|
...
| Saved workflows | A template can carry a pipeline graph instead of a single tool, so a multi-step workflow with parallel branches, per-step tools, and retries is one saved object that a launch, a schedule, or a webhook fires the same way, and its survey answers reach every step.|
...
| Worker queues (Team) | Target a run at a named queue. A worker serving that queue runs it and default workers leave it alone. Pin a queue on a run, a template, or an inventory, most specific wins, so queues work like AWX instance groups. A queue is grantable as `queue:<name>` and matchable in a policy, so choosing one is access-controlled on both sides rather than only on the worker's.|
...
| Teams and grants | Group users into teams and grant read, use, or manage on a specific project, template, inventory, or credential, each level implying the ones below it. A read grant delegates view of one object without a global viewer role; a manage grant delegates editing and deleting without global admin. Grants layer on the global role and default open, so an ungranted object falls back to the caller's global role: separation between organizations is not enforced until `--strict-grants` is on.|
| Reach isolated networks (Team) | A worker dials out to the control node over the mesh relay with a token and needs no inbound port, so it runs jobs inside an air-gapped segment, a DMZ, or a customer network the control node cannot reach directly. Point `switchtender worker --server` at the control node and set the worker token.|
...
| Drift reconcile (Team) | One click on a drifted target builds the fix, run for real and born held for approval: an Ansible host reruns its playbook limited to that host, a Terraform directory applies. A machine proposes, a person releases, and the audit trail records both.|
...
| Migration | `switchtender import awx` and `import semaphore` read an export and create the equivalent projects, inventories, templates, surveys, schedules, and credential shells, with a dry-run report first. `import chef` and `import puppet` bring a desired-state fleet across as an inventory, grouped by environment and role, since those systems hold no job definitions to carry and their cookbooks and manifests do not convert. An AWX workflow job template arrives as a workflow template and keeps its own schedules, unless its graph is one a pipeline cannot express, such as a node that runs other nodes on failure, which is reported and skipped whole rather than imported in part. `switchtender import rundeck` reads either a job export or a project archive, told apart by content, converting Quartz schedules and refusing to downgrade secure options to plaintext. Neither Rundeck artifact carries a node definition, so no inventory is imported from either and each node source is reported by kind and location instead. An archive brings one project when its source control configuration names a repository this can reach. `switchtender import jenkins` reads a Jenkins jobs directory or a zip of one, turning freestyle jobs into Bash templates, resolving Jenkins H notation into real cron times, and refusing to import a password parameter or a poll trigger. `switchtender import cron` brings a crontab under governance, turning each cron line into an approved, recorded schedule. The per-source table is in [what each source brings over](migration.md#what-each-source-brings-over).|
| Offboarding record | A schedule and a webhook trigger each record who created it. Neither stops firing when that account is deleted, because a schedule is organization infrastructure and a trigger's token belongs to the trigger, so halting production work the moment somebody leaves would be its own outage. The record is what an offboarding review needs to decide which to reassign or rotate.|
| Schedule overlap | A schedule waits for its own previous run. A five-minute schedule whose playbook takes eight minutes skips the tick rather than stacking a second copy of the same work on the same hosts, and stays on its cadence rather than firing the moment the slow run ends. Each skip is logged.|
| Schedule timezones | A schedule reads its cron expression in an IANA timezone such as `America/New_York`, following that zone's daylight-saving shifts, so a nightly window stays put across the year instead of drifting with the server clock. On the day a zone falls back the repeated hour fires once, not twice, and on the day it springs forward a slot inside the hour that never happens fires at the instant the clocks jumped, so a nightly job runs every night of the year.|
