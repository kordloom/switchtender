...
| Schedule timezones | A schedule reads its cron expression in an IANA timezone such as `America/New_York`, following that zone's daylight-saving shifts, so a nightly window stays put across the year instead of drifting with the server clock. On the day a zone falls back the repeated hour fires once, not twice. A slot inside the hour a zone skips forward does not exist that day and does not fire, so avoid scheduling in the transition hour if the run must happen every day.|
