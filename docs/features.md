...
| Scheduled evidence packs | `--evidence-dir` with `--evidence-cadence` writes a change register covering each elapsed period, named for the period it covers, so an archive sorts chronologically and a missing period is a missing file. The evidence a review samples from exists before it is asked for.|
