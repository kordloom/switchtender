...
| Launch receipts | Every run records the `seq:link` of the chain entry that authorized its creation, so who asked for a run is redeemable evidence rather than a field the server asserts. A creation entry the chain can no longer produce is reported.|
