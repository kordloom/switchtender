...
func (f *fakeSubmitter) Submit(context.Context, string, string, ...string) (*run.Run, error) {
	f.calls++
	f.kind = "single"
...
func (f *fakeSubmitter) SubmitSplit(context.Context, string, string, int, ...string) (*run.Run, error) {
	f.calls++
	f.kind = "split"
	return &run.Run{ID: f.runID}, nil
}
...
func (f *fakeSubmitter) SubmitPipeline(context.Context, string, string, []run.PipelineStep, ...string) (*run.Run, error) {
	f.calls++
	f.kind = "pipeline"
	return &run.Run{ID: f.runID}, nil
}
