...
// grantStore is a grant.Store backed by the shared PostgreSQL database.
...
func (s *grantStore) Get(ctx context.Context, id string) (*grant.Grant, error) {
	const q = "SELECT " + grantColumns + " FROM grants WHERE id=$1"
...
func (s *grantStore) Delete(ctx context.Context, id string) error {
	res, err := s.db.ExecContext(ctx, "DELETE FROM grants WHERE id=$1", id)
	if err != nil {
...
func (s *grantStore) ForObject(ctx context.Context, object string) ([]*grant.Grant, error) {
	const q = "SELECT " + grantColumns + " FROM grants WHERE object=$1 ORDER BY created_at, id"
