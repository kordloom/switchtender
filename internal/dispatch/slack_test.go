...
func TestSlackMessage(t *testing.T) {
	...
	tests := []struct {
		...
		Want: ":white_check_mark: Yardmaster run *deploy.yml* succeeded in 1m30s",
		...
		Want: ":x: Yardmaster run *deploy.yml* failed in 1m30s\n> exit status 2",
		...
		Want: ":white_check_mark: Yardmaster run *terraform apply* succeeded",
	}, { // Test 3: A run with no label falls back to the id.
		In:   &run.Run{ID: "r-1", Status: run.StatusSucceeded},
		Want: ":white_check_mark: Yardmaster run *r-1* succeeded",
	}}
