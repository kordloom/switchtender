...
func TestRelaunchFailedHosts(t *testing.T) {
	...
	src := &run.Run{
		...
		Tags: []string{"deploy"}, Status: run.StatusRunning, CreatedAt: time.Now(),
	}
	...
	relaunch, err := d.RelaunchFailedHosts(ctx, "run_src")
	if err != nil {
	...
	_ = store.Save(ctx, green)
	if _, err := d.RelaunchFailedHosts(ctx, "run_green"); !errors.Is(err, ErrNoFailedHosts) {
	...
	if _, err := d.RelaunchFailedHosts(ctx, "run_bash"); !errors.Is(err, ErrNoHostSummary) {
