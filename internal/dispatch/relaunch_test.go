...
func TestRelaunchFailedHosts(t *testing.T) {
	...
	relaunch, err := d.RelaunchFailedHosts(ctx, "run_src", run.WithActor("operator_b"), run.WithActorType("session"))
	if err != nil {
	...
	_ = store.Save(ctx, green)
	if _, err := d.RelaunchFailedHosts(ctx, "run_green", run.WithActor("operator_b"), run.WithActorType("session")); !errors.Is(err, ErrNoFailedHosts) {
	...
	if _, err := d.RelaunchFailedHosts(ctx, "run_bash", run.WithActor("operator_b"), run.WithActorType("session")); !errors.Is(err, ErrNoHostSummary) {
