...
func (s *store) PurgeRunsBefore(ctx context.Context, cutoff time.Time) (int, error) {
	...
	// absent. INSERT OR IGNORE, so a re-run of a partially completed purge is harmless.
	if _, err := s.db.ExecContext(ctx, `
INSERT OR IGNORE INTO run_auth (run_id, org_id, project_id, inventory_id, pull_credential_id, credential_ids)
SELECT id, org_id, project_id, inventory_id, pull_credential_id, credential_ids
FROM runs WHERE `+terminalRun+` AND created_at < ?`, cut); err != nil {
...
// whole table. A single ranked delete has to be batched to avoid holding the writer, and every
// batch would re-rank every row, so clearing a large backlog would scan the table once per batch.
// Grouping first costs one index-only pass and then touches only the keys that are actually over
// the limit, and each delete rides the ordered index for one key.
...
func (s *store) trimSummaryTable(ctx context.Context, trim summaryTrim, keep int) (int, error) {
	over := fmt.Sprintf(
		"SELECT %s FROM %s GROUP BY %s HAVING COUNT(*) > ?", trim.column, trim.table, trim.column)
	...
	del := fmt.Sprintf(`
DELETE FROM %s WHERE %s = ? AND (run_id, %s) NOT IN (
	SELECT run_id, %s FROM %s WHERE %s = ?
	ORDER BY `+sqlutil.TimeOrder+` DESC, run_id DESC LIMIT ?
)`, trim.table, trim.column, trim.column, trim.column, trim.table, trim.column)
	deleted := 0
	for _, key := range keys {
		res, err := s.db.ExecContext(ctx, del, key, key, keep)
		if err != nil {
			return deleted, err
		}
		n, err := res.RowsAffected()
