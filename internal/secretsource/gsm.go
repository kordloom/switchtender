...
type gsmConfig struct {
	...
	// SwitchTender running on GCP reads as its attached service account.
...
// the base64 payload. Without a config token it needs SwitchTender to run on GCP, since the metadata
...
// gsmMetadataToken fetches an OAuth2 access token from the GCP metadata server, so a SwitchTender on
