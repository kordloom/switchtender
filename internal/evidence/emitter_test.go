...
func TestProgressResumesFromTheArchiveNotFromMemory(t *testing.T) {
	...
	fresh := NewEmitter(runs, audits, audit.Identity{}, t.TempDir(), time.Hour, nil)
	defer fresh.Close()
...
func TestEmitNamesTheDirectoryItCouldNotUse(t *testing.T) {
	...
	e := NewEmitter(runs, audits, audit.Identity{}, filepath.Join(file, "packs"), time.Hour, nil)
	defer e.Close()
...
func TestNewEmitterRefusesConfigurationThatCannotWork(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			NewEmitter(runs, audits, audit.Identity{}, tc.Dir, tc.Cadence, nil)
		})
	}
}
